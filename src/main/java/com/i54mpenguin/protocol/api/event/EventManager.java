package com.i54mpenguin.protocol.api.event;

import com.google.common.base.Preconditions;
import com.google.common.collect.Lists;
import com.i54mpenguin.protocol.api.handler.PacketListener;
...
//            PunisherPlugin.getInstance().getLogger().warning("[Protocol] Handling inbound packet while channel not initialized.");
            return null;
        }
        ...
        listeners.stream().sorted(Comparator.comparingInt(PacketListener::getPriority)).filter(it -> {
            ...
            try {
                ...
                PunisherPlugin.getInstance().getLogger().log(Level.SEVERE, "[Protocol] Exception caught in listener while receiving packet "+packet.getClass().getName(), e);
            }
        });
...
//            ProxyServer.getInstance().getLogger().warning("Handling outbound packet while channel not initialized.");
        ...
        listeners.stream().sorted(Comparator.comparingInt(PacketListener::getPriority)).filter(it -> {
            ...
            try {
                ...
                PunisherPlugin.getInstance().getLogger().log(Level.SEVERE, "[Protocol] Exception caught in listener while sending packet "+packet.getClass().getName(), e);
            }
        });
        if(shouldFireBungeeEvent()) {
            ProxyServer.getInstance().getPluginManager().callEvent(event);
