...
public class AdminChat extends Command {
    ...
    public void execute(CommandSender commandSender, String[] strings) {
        if (commandSender instanceof ProxiedPlayer) {
            ...
            if (strings.length == 0) {
                player.sendMessage(new ComponentBuilder("Send a message to all admins").color(ChatColor.RED).append("\nUsage: /adminchat <message>").color(ChatColor.WHITE).create());
                return;
            }
    ...
    public static void sendChatMessage(BaseComponent[] message, ProxiedPlayer player){
        ...
        messagetosend = new ComponentBuilder(ChatColor.translateAlternateColorCodes('&', prefix.replace("%server%", player.getServer().getInfo().getName()).replace("%player%", userPrefix + ChatColor.RESET + " " + color + ChatColor.RESET + player.getName()))).event(hover).append(message).color(color).create();
